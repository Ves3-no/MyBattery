...
function LogCharging({ShowPopUp, setShowPopUp, setBattery, setKmSinceLastCharge, setDateOfLastCharge }: {ShowPopUp:boolean, setShowPopUp:Dispatch<SetStateAction<boolean>>, setBattery:Dispatch<SetStateAction<number>>, setKmSinceLastCharge:Dispatch<SetStateAction<number>>, setDateOfLastCharge:Dispatch<SetStateAction<DateOwn>>}){
    const [NewBattery, setNewBattery] = useState<number>(0);
    return(
        <>
            <div style={{display: ShowPopUp ? "flex" : "none", zIndex: ShowPopUp ? "100" : "-100"}} className="absolute bg-gray-800/50 w-full h-full top-0 items-center justify-center">
