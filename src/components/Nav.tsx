...
function Nav({KmStand, SeasonNow, Battery, setBattery, setKmStand, AvrageKmThisSeason, KmSinceLastCharge, setKmSinceLastCharge, setPrevTrips, PrevTrips, setShowBatteryPopUp}: {
  KmStand: number;
  SeasonNow: Season;
  Battery: number;
    ...
    return(
        <>
            <nav className="h-18 bg-main w-full fixed bottom-0 z-10 flex items-center justify-around px-4 p-2 z-200">
                <button className="text-main-light hover:text-main-dark" onClick={()=> {setShowBatteryPopUp(false); setShowPupUp(false)}}><Home fill="currentColor"/></button>
